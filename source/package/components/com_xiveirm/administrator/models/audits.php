 ...
 */
class XiveirmModelaudits extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		parent::populateState('a.item_id', 'asc');
	}
	...
	protected function getListQuery()
	{
		...
		$query->from('`#__xiveirm_audit` AS a');
		...
		if (!empty($search)) {
			if (stripos($search, 'id:') === 0) {
                ...
                $query->where('( a.item_id LIKE '.$search.'  OR  a.reference_key LIKE '.$search.'  OR  a.reference_value LIKE '.$search.' )');
			}
		}
