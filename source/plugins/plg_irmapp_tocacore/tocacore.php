						...
						<small>
							<?php
								if($item->created && $item->created != '0000-00-00 00:00:00') {
									echo JText::_('PLG_IRMAPP_TOCACORE_CREATED') . ': ' . date(JText::_('DATE_FORMAT_LC2'), strtotime($item->created)) . '<br>';
								}
								if($item->modified && $item->modified != '0000-00-00 00:00:00') {
									echo JText::_('PLG_IRMAPP_TOCACORE_LAST_MODIFIED') . ': ' . date(JText::_('DATE_FORMAT_LC2'), strtotime($item->modified));
								} else {
